...
plt.ylim(0.00, 0.30)
plt.xticks(Sharpe_R.index, Sharpe_R.index, color='black', rotation=0,  fontsize='10', horizontalalignment='left')
for x, y in zip(Sharpe_R.index, Sharpe_R):
    # ha: horizontal alignment
    # va: vertical alignment
    plt.text(x, y+0.02,'%.2f' %y, ha='left', va='top')
...
plt.ylim(0.00, 0.4)
plt.xticks(Sortino_R.index, Sortino_R.index, color='black', rotation=0,  fontsize='10', horizontalalignment='left')
for x, y in zip(Sortino_R.index, Sortino_R):
    # ha: horizontal alignment
    # va: vertical alignment
    plt.text(x , y+0.03, '%.2f' % y, ha='left', va='top')
...
plt.ylim(-0.5, 0.65)
plt.xticks(Jensen_Alpha.index, Jensen_Alpha.index, color='black', rotation=0,  fontsize='10', horizontalalignment='left')
for x, y in zip(Jensen_Alpha.index, Jensen_Alpha):
    if y >= 0:
        plt.text(x , y+0.07, '%.2f' % y, ha='left', va='top')
    elif y < 0:
        plt.text(x , y-0.07, '%.2f' % y, ha='left', va='bottom')
...
plt.ylim(-0.6, 0.65)
plt.xticks(Three_Factor_Alpha.index, Three_Factor_Alpha.index, color='black', rotation=0,  fontsize='10', horizontalalignment='left')
for x, y in zip(Three_Factor_Alpha.index, Three_Factor_Alpha):
    if y >= 0:
        plt.text(x , y+0.08, '%.2f' % y, ha='left', va='top')
    elif y < 0:
        plt.text(x , y-0.08, '%.2f' % y, ha='left', va='bottom')
...
plt.plot(minstd, mu, color='k', label='Efficient Frontier from Assignment1', linewidth=1.0, linestyle='--')
...
plt.grid(True)
plt.savefig('Simulation_final.jpeg', dpi=300)
plt.show()
