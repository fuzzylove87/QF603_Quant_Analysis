...
plt.title('Black Scholes movement with N = 21')
plt.plot(t,x)
plt.show()
...
plt.figure()
plt.hist(error_21, weights=(np.zeros_like(np.array(error_21))+1/ np.array(error_21).size)*100,bins=100)
...
plt.hist(error_84, weights=(np.zeros_like(np.array(error_84))+1/ np.array(error_84).size)*100,bins=100)
